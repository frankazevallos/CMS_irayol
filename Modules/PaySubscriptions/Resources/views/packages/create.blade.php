...
<div class="card">
    <div class="card-header clearfix">        
        ...
        <div class="btn-group-sm float-right" role="group">
            <a href="{{ route('paysubscriptions.index') }}" class="btn btn-primary" title="{{__('global.return_back')}}">
                <i class="fa fa-undo" aria-hidden="true"></i> {{__('paysubscriptions::global.return_back')}}
            </a>
        </div>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('packages.store') }}" accept-charset="UTF-8" id="create_package_form" name="create_package_form" class="form-horizontal">
            {{ csrf_field() }}
            @include ('paysubscriptions::packages.form', ['package' => null,])
